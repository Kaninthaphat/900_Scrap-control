...
public class view_916_SCRAP_ADD {
...
//	private int sc_big_type = 0;
	...
	public static void setPath_name(String path_name) {
		view_916_SCRAP_ADD.path_name = path_name;
	}
	...
	{		
		...
		try{			                 
			...
			if(id == 0){
			    ...
			    if(factory_id != 0 && pd_group_id != 0 && sc_type_id != 0 && scrap_Weight !=0){				    					
			    	
			    	
			    	java.sql.CallableStatement cstmt = conn.prepareCall("{call [905_SCRAPFORM_INS_IN](?,?,?,?,?,?,?,?,?,?,?)}");
					
					cstmt.setInt(1, id);
					cstmt.setString(2, DateTime.set_DateString(scrap_date_d));
					cstmt.setInt(3, pd_group_id);
					cstmt.setInt(4, sc_type_id);
					cstmt.setDouble(5, scrap_Quatity);
					cstmt.setInt(6, scrap_unit);
					cstmt.setDouble(7, scrap_Weight);
					cstmt.setString(8, scrap_remark);
					cstmt.setString(9, user_login);
					cstmt.setString(10, user_login);
					cstmt.setInt(11, factory_id);
					cstmt.execute();
					...
					Clients.showNotification("WARNING_MESSAGE.!  <br/> <br>*Factory, <br>*Group Product, <br>*Scrap Type, <br>*Weight </br><br/></br></br>",
				    Clients.NOTIFICATION_TYPE_WARNING, null, "middle_center", 3100);
				}
	...
	{				
	        ...
	        SqlSelection sqlsel = new SqlSelection();
	        String StrSQL = "EXEC [905_SCRAP_FORM_Q_IN] @START_DATE = '"+startDate_string+"',"
	        		   ...
	        		   if(item_h.getStatus_id() == 1 || item_h.getStatus_id() == 2){
	        			 ...
	        			 if(sum_row == 1 || sum_row == 2){
	        			   ...
	        			   if(i_row % 2 == 0){
	        				...
	        				{
		        					java.sql.CallableStatement cstmt = conn.prepareCall("{call [906_SCRAPFORM_CANCEL_IN](?,?,?)}");			               		
...
//				                }
    ...
    private void PrintToXls() throws IOException{									
		...
		excel.createSheet("Scrap_Control");
			...
			excel.setColumnWidth(12, 5);
			excel.setColumnWidth(13, 4);
			excel.setColumnWidth(14, 5);
			...
			excel.setCellValue(1, 5, "Scrap Type", excel.getStyle("HEAD"));
			excel.setCellValue(1, 6, "Remark", excel.getStyle("HEAD"));
			excel.setCellValue(1, 7, "Quatity", excel.getStyle("HEAD"));
			excel.setCellValue(1, 8, "Unit", excel.getStyle("HEAD"));
			excel.setCellValue(1, 9, "Weight(Kgs.)", excel.getStyle("HEAD"));
			excel.setCellValue(1, 10, "Status", excel.getStyle("HEAD"));	
			excel.setCellValue(1, 11, "CreateDate", excel.getStyle("HEAD"));	
			excel.setCellValue(1, 12, "CreateBy", excel.getStyle("HEAD"));	
			excel.setCellValue(1, 13, "LastUpdate", excel.getStyle("HEAD"));	
			excel.setCellValue(1, 14, "UpdateBy", excel.getStyle("HEAD"));	
			...
			for(model_scrap_inform report : model_list_scrap){
				...
				excel.setCellValue(rowNum, 1, report.getId(), excel.getStyle("DC"));
				excel.setCellValue(rowNum, 2, report.getScrap_date(), excel.getStyle("DR"));		
				excel.setCellValue(rowNum, 3, report.getFac_name(), excel.getStyle("DR"));
				excel.setCellValue(rowNum, 4, report.getPd_group_name(), excel.getStyle("DR"));	
				excel.setCellValue(rowNum, 5, report.getScraptype_name(), excel.getStyle("DR"));
				excel.setCellValue(rowNum, 6, report.getScrap_remark(), excel.getStyle("DR"));					
				excel.setCellValue(rowNum, 7, report.getScrap_Quatity(), excel.getStyle("DC"));	
				excel.setCellValue(rowNum, 8, report.getScp_unit_name(), excel.getStyle("DR"));	
				excel.setCellValue(rowNum, 9, report.getScrap_Weight(), excel.getStyle("DR"));	
				excel.setCellValue(rowNum, 10, report.getStatus_name2(), excel.getStyle("DR"));	
				excel.setCellValue(rowNum, 11, report.getCreate_date(), excel.getStyle("DR"));	
				excel.setCellValue(rowNum, 12, report.getCreate_name(), excel.getStyle("DR"));	
				excel.setCellValue(rowNum, 13, report.getUpdate_date(), excel.getStyle("DR"));	
				excel.setCellValue(rowNum, 14, report.getUpdate_name(), excel.getStyle("DR"));	
						
				rowNum ++;
			}
			
			excel.setBlankRow(rowNum, 0, 14, excel.getStyle("LB"));
